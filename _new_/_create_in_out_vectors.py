...
def create_in_out_vectors(docs, dictionary):
    ...
    for i in range(len(docs)):
        ...
        # Replace words from document not included in our dictionary by 0 or __OOV__
        fv = []
        fv_nn = []
        for word in dictionary:   # v tvare list slovnikov [{2 : slovoX}, {5 : slovoY}...]
            if word[-1] in wordlist:  # slovoX
                ...
                fv_nn.append(((count/len_wordlist)*100))   # vyskyt daneho slova v povodnom dokumente v % #TODO v povodnom alebo uz bez stop slov
            else:
                # fv.append('__OOV__')
                fv_nn.append(0.)
        ...
        #### OUT/LV ####
        # na koniec FV pripojime LABEL=TOPIC dokumentu (na akej pozicii sa nachadza topic v zozname vsetkych topicov, je UNIQUE)
        doc_topic = reuters.categories(docs[i])
        topic_id = all_topics.index(doc_topic[0])
        fv_nn.append(topic_id)

        # V kazdom cykle spracujeme 1 dokument a prilepime jeho vektor vlastnosti + topic id k testovacim datam
        test_data.append(fv_nn)

    return test_data  # all Feature Vectors + Labels as list