...
TRAINING_DATA = "train_data_5000.csv"
...
def main():
    ...
    for network in networks:
        ...
        classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns,
                                                ...
                                                model_dir="tmp/rt_model" + str(networks.index(network)),
        ...
        print("{:8.4f}".format(max_test*100) + " ({:8.4f}), ".format(max_train*100) + f"step: {accuracy_score_test.index(max_test)} " + f"({accuracy_score_train.index(max_train)}), " + str(network))
